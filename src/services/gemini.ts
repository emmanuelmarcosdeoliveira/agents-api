...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Transcreva o áudio para o português do Brasil. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida o texto em parágrafos quado for apropriado ',
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  const prompt = `
    Com base no texto fornecido abaixo como contexto, responda a pergunta  de forma clara a precisa em português do Brasil. 
